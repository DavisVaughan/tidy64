...
warn_to_int_from_tidy64_oob <- function(x) {
  warn_tidy64(x = x, class = "tidy64_warning_to_int_from_tidy64_oob")
}

#' @export
cnd_header.tidy64_warning_to_int_from_tidy64_oob <- function(cnd, ...) {
  "Input is outside the range of an integer."
}

#' @export
cnd_body.tidy64_warning_to_int_from_tidy64_oob <- function(cnd, ...) {
  indicator <- tidy64_detect_to_int_from_tidy64_oob(cnd$x)
  locations <- which(indicator)
